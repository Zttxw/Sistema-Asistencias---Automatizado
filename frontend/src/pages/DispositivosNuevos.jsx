  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 dark:border-zinc-800 pb-5">
        <div>
          <div className="flex items-center space-x-2">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Dispositivos Nuevos</h2>
            <span className="w-2.5 h-2.5 rounded-full bg-accent animate-pulse" title="Escáner Activo"></span>
          </div>
          <p className="text-sm text-gray-500 dark:text-zinc-400 mt-1">Dispositivos Wi-Fi detectados en la red no asociados a un empleado.</p>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <div className="flex items-center space-x-2 bg-white dark:bg-black border border-gray-200 dark:border-zinc-800 rounded-lg px-3 py-2 text-sm shadow-2xs">
            <span className="text-xs text-gray-500 dark:text-zinc-400 font-medium">Vistos en los últimos:</span>
            <select
              value={minutos}
              onChange={(e) => setMinutos(Number(e.target.value))}
              className="bg-transparent border-none text-gray-800 dark:text-zinc-200 focus:outline-none cursor-pointer font-medium"
            >
      ...
      <div className="bg-white dark:bg-black border border-gray-100 dark:border-zinc-800 rounded-xl shadow-2xs overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-sm text-gray-400 dark:text-zinc-500">Escaneando red...</div>
        ) : dispositivos.length === 0 ? (
          <div className="p-12 text-center text-sm text-gray-400 dark:text-zinc-500 flex flex-col items-center justify-center space-y-2">
            <Radio className="w-8 h-8 text-gray-300 dark:text-zinc-700 stroke-1" />
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-zinc-800">
              {dispositivos.map((item, idx) => (
                <tr key={idx} className="hover:bg-gray-50/50 dark:hover:bg-zinc-900/50 transition-colors">
                  <td className="px-6 py-4 font-mono text-xs font-semibold text-gray-900 dark:text-white flex items-center space-x-2">
                    <span className="w-2 h-2 rounded-full bg-accent shrink-0"></span>
                    <span>{item.mac}</span>
                  </td>
                  <td className="px-6 py-4 text-gray-600 dark:text-zinc-300 font-medium">{item.fabricante || 'Desconocido'}</td>
                  <td className="px-6 py-4 text-gray-500 dark:text-zinc-400 font-mono text-xs">{formatDateTime(item.primera_vez_visto)}</td>
                  <td className="px-6 py-4 text-gray-500 dark:text-zinc-400 font-mono text-xs">{formatDateTime(item.ultima_vez_visto)}</td>
                  <td className="px-6 py-4 text-gray-700 dark:text-zinc-200 font-mono text-xs font-semibold">{item.veces_visto}</td>
      ...
      >
        <form onSubmit={handleRegister} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-gray-700 dark:text-zinc-300 mb-1">Dirección MAC (Detección en Vivo)</label>
            <input
              type="text"
              readOnly
              value={selectedMac}
              className="w-full border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-950 rounded-lg px-3 py-2 text-sm text-gray-600 dark:text-zinc-300 font-mono cursor-not-allowed uppercase font-semibold"
            />
          </div>
          <div>
